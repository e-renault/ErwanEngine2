...
__kernel void simpleCast (
    ...
    //temp
    rgb color_value = {0.75, 0.87, 0.93};//default sky
    Vector3 directive_light_v = {0.6, 0.7, 1};//default sky illumination
    Vector3 up = {0, 1, 0};//constant vector
    ...
    //compute proof color
    float min = 2.0, max = 6.0;
    float grad;
    if (z_value<min) {
        grad = 1;
    } else {
        grad = 1-exp(-1/(-min+z_value));
    }
    ...
    float a = 3;
    direct_light = direct_light*(a-1)/a + 1/a;
        ...
        for (j=0; j<SSAO_nb_iteraton; j++) {
            ...
            for(i = 0; i<nb_triangle; i++) {
                ...
                if (hit && getLength(global_pos, point_buffer) < 0.2) {
                    counter-=abs_diff(5, getLength(global_pos, point_buffer));
                    break;
                }
    ...
    final_outut_buffer[y*x_res + x].r = SSAO_light * direct_light * color_value.r;// + (1-grad);
    final_outut_buffer[y*x_res + x].g = SSAO_light * direct_light * color_value.g;// + (1-grad);
    final_outut_buffer[y*x_res + x].b = SSAO_light * direct_light * color_value.b;// + (1-grad);
} 
